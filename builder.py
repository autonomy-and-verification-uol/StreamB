...
class StreamBuilder(StreamVisitor):
    ...
    # create the launch file for all the transducers generated by the builder
    def createLaunch(self):
        statements = []
        statements += ['<launch>']
        #statements += ['\t<node pkg="transducer" type="clockNode.py" name="clockNode" output="screen"/>']
        for filename in os.listdir('./transducers/'):
            statements += ['\t<node pkg="stream" type="{fn}" name="{id}" output="screen"/>'.format(fn=filename, id=filename[:-3])]
            st = os.stat('./transducers/' + filename)
            os.chmod('./transducers/' + filename, st.st_mode | stat.S_IEXEC)
        statements += ['</launch>']
        source = '\n'.join(statements)
        with open('./transducers/run.launch', 'w') as launch_file:
            launch_file.write(source)

    # create a composition transducer
    def createCompositiontransducer(self, transducerName, labelL, labelR, typeL, typeR, operator):
        ...
        # if the left component is composed, we have to subscribe to its transducer
        ...
        statements += ['\tglobal pub, transducer']
        statements += ['\trospy.init_node(\'{transducerName}\', anonymous=True)'.format(transducerName=transducerName)]
        statements += ['\tpub = rospy.Publisher(name = \'{topic}\', data_class = {ty}, latch = True, queue_size = 1000)'.format(topic=transducerName, ty=self.type)]
        if labelL.startswith('kwargs'):
        ...
        with open('./transducers/' + transducerName + '.py', 'w') as transducer:
            transducer.write(source)

    # create the aggregation transducer to aggregate the result obtained by a lower level transducer
    def createAggregationtransducer(self, transducerName, aggrFunc, type, child):
        # imports necessary
        statements = []
        statements += ['#!/usr/bin/env python']
        statements += ['import rospy']
        ...
        statements += [aggrFunc]
        # we add the callback to listen to the results obtained by the lower level transducer
        ...
        statements += ['\tglobal pub, transducer']
        statements += ['\trospy.init_node(\'{transducerName}\', anonymous=True)'.format(transducerName=transducerName)]
        statements += ['\tpub = rospy.Publisher(name = \'{topic}\', data_class = {type}, latch = True, queue_size = 1000)'.format(topic=transducerName, type=type)]
    ...
    # all opur transducers follow the clock to intercept and propagate the information
    def createClockNode(self):
        ...
        with open('./transducers/clockNode.py', 'w') as transducer:
            transducer.write(source)

    # create a MTL transducer
    def createMTLtransducer(self):
        classname = 'mtl_transducer_' + str(self.ntransducers)
        ...
        statements += ['ws_lock = Lock()']
        statements += ['']
        statements += ['class Basetransducer:']
        ...
        statements += ['']
        statements += ['class {classname}(Basetransducer):'.format(classname=classname)]
        statements += ['']
        statements += ['\ttime = -1']
        ...
        # extract the topics of interest (we need to subscribe to all the lower transducers providing the info we need in the MTL property)
        subscribers = set()
        ...
        for subscriber in subscribers:
            ...
            statements += ['\tws_lock.release()']
        statements += ['']
        # we need a conditional publish as for the composition transducer
        ...
        statements += ['\t\tmsg = TimedBool()']
        statements += ['\t\tmsg.time = dict_msgs.peekitem(0)[0]']
        statements += ['\t\tmsg.value = transducer.update(**kw)']
        statements += ['\t\tpub.publish(msg)']
        statements += ['\t\tdict_msgs.popitem(0)']
        statements += ['\t\tattempts = 0']
        ...
        statements += ['\ttransducer = {classname}()'.format(classname=classname)]
        for subscriber in subscribers:
        ...
        with open('./transducers/mtl_transducer_' + str(self.ntransducers) + '.py', 'w') as transducer:
            transducer.write(source)

        self.ntransducers += 1
        return 'kwargs[\'mtl_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitNamedExpr(self, ctx:StreamParser.NamedExprContext):
        ...
        for expr in ctx.expr():
            ...
            # create the high level transducer which publish on name
            transducerName = 'transducer_' + str(ctx.IDENTIFIER(i))
            statements = []
            statements += ['#!/usr/bin/env python']
            statements += ['import rospy']
            statements += ['import intervals']
            ...
            statements += ['from std_msgs.msg import *']
            statements += ['']
            # create the callback on the result for the lower level transducer
            ...
            #self.ntransducers += 1
            i += 1
        return 'kwargs[\'transducer_' + str(self.ntransducers) + '\']'
    ...
    def visitAtomicExpression(self, ctx:StreamParser.AtomicExpressionContext):
        ...
        if type(ctx.child) is StreamParser.EvaluationContext:
            ...
            return self.createMTLtransducer()
    ...
    def visitAnd(self, ctx:StreamParser.AndContext):
        ...
        if isinstance(ctx.left, (StreamParser.EvaluationContext, StreamParser.AndContext, StreamParser.OrContext)):
            ...
            labelL = self.createMTLtransducer() # create the corresponding MTL transducer
        typeL = self.type
        self.reset()
        ...
        if isinstance(ctx.left, (StreamParser.EvaluationContext, StreamParser.AndContext, StreamParser.OrContext)):
            ...
            labelR = self.createMTLtransducer() # create the corresponding MTL transducer
        typeR = self.type
        operator = ' and '
        self.type = 'TimedBool'
        if labelL + operator + labelR in self.already_seen:
            ...
            # create the composition transducer for and
            self.createCompositiontransducer('and_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, operator)
            self.ntransducers += 1
            self.already_seen[labelL + operator + labelR] = 'kwargs[\'and_transducer_' + str(self.ntransducers - 1) + '\']'
            return 'kwargs[\'and_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitOr(self, ctx:StreamParser.OrContext):
        ...
        if labelL + operator + labelR in self.already_seen:
            ...
            # create the composition transducer for and
            self.createCompositiontransducer('or_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, operator)
            self.ntransducers += 1
            self.already_seen[labelL + operator + labelR] = 'kwargs[\'or_transducer_' + str(self.ntransducers - 1) + '\']'
            return 'kwargs[\'or_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitLess(self, ctx:StreamParser.LessContext):
        ...
        if labelL + operator + labelR in self.already_seen:
            ...
            self.createCompositiontransducer('less_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, ' < ')
            self.already_seen[labelL + operator + labelR] = 'less_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'less_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitLessEq(self, ctx:StreamParser.LessEqContext):
        ...
        if labelL + operator + labelR in self.already_seen:
            ...
            self.createCompositiontransducer('less_eq_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, operator)
            self.already_seen[labelL + operator + labelR] = 'less_eq_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'less_eq_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitGreater(self, ctx:StreamParser.GreaterContext):
        ...
        if labelL + operator + labelR in self.already_seen:
            ...
            self.createCompositiontransducer('greater_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, operator)
            self.already_seen[labelL + operator + labelR] = 'greater_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'greater_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitGreaterEq(self, ctx:StreamParser.GreaterEqContext):
        ...
        if labelL + operator + labelR in self.already_seen:
            ...
            self.createCompositiontransducer('greater_eq_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, operator)
            self.already_seen[labelL + operator + labelR] = 'greater_eq_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'greater_eq_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitEq(self, ctx:StreamParser.EqContext):
        ...
        if labelL + operator + labelR in self.already_seen:
            ...
            self.createCompositiontransducer('eq_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, operator)
            self.already_seen[labelL + operator + labelR] = 'eq_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'eq_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitNeq(self, ctx:StreamParser.NeqContext):
        ...
        if labelL + operator + labelR in self.already_seen:
            ...
            self.createCompositiontransducer('neq_transducer_' + str(self.ntransducers), labelL, labelR, typeL, typeR, operator)
            self.already_seen[labelL + operator + labelR] = 'neq_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'neq_transducer_' + str(self.ntransducers - 1) + '\']'

    # visit (min expr)
    def visitMin(self, ctx:StreamParser.MinContext):
        label = self.visit(ctx.child)
        # create the aggregation function to pass to the aggregation transducer creator
        statements = []
        ...
        if 'min' + label in self.already_seen:
            ...
            self.createAggregationtransducer('min_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['min' + label] = 'min_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'min_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitTimedMin(self, ctx:StreamParser.TimedMinContext):
        ...
        if 'tmin' + interval + label in self.already_seen:
            ...
            self.createAggregationtransducer('tmin_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['tmin' + interval + label] = 'tmin_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'tmin_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitMax(self, ctx:StreamParser.MaxContext):
        ...
        if 'max' + label in self.already_seen:
            ...
            self.createAggregationtransducer('max_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['max' + label] = 'max_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'max_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitTimedMax(self, ctx:StreamParser.TimedMaxContext):
        ...
        if 'tmax' + interval + label in self.already_seen:
            ...
            self.createAggregationtransducer('tmax_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['tmax' + interval + label] = 'tmax_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'tmax_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitAvg(self, ctx:StreamParser.AvgContext):
        ...
        if 'avg' + label in self.already_seen:
            ...
            print('HERE3')
            self.createAggregationtransducer('avg_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['avg' + label] = 'avg_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'avg_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitTimedAvg(self, ctx:StreamParser.TimedAvgContext):
        ...
        if 'tavg' + interval + label in self.already_seen:
            ...
            self.createAggregationtransducer('tavg_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['tavg' + interval + label] = 'tavg_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'tavg_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitTimedDiff(self, ctx:StreamParser.TimedDiffContext):
        ...
        if 'tdiff' + interval + label in self.already_seen:
            ...
            self.createAggregationtransducer('tdiff_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['tdiff' + interval + label] = 'tdiff_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'tdiff_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitTwoStreamDiff(self, ctx:StreamParser.TwoStreamDiffContext):
        ...
        if labelF + operator + labelS in self.already_seen:
            ...
            self.createCompositiontransducer('two_stream_diff_transducer_' + str(self.ntransducers), labelF, labelS, typeF, typeS, operator)
            self.already_seen[labelF + operator + labelS] = 'two_stream_diff_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'two_stream_diff_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitDelta(self, ctx:StreamParser.DeltaContext):
        ...
        if 'delta' + label in self.already_seen:
            ...
            self.createAggregationtransducer('delta_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['delta' + label] = 'delta_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'delta_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitTimedDelta(self, ctx:StreamParser.TimedDeltaContext):
        ...
        if 'tdelta' + interval + label in self.already_seen:
            ...
            self.createAggregationtransducer('tdelta_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['tdelta' + interval + label] = 'tdelta_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'tdelta_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitCount(self, ctx:StreamParser.CountContext):
        ...
        if 'count' + label in self.already_seen:
            ...
            self.createAggregationtransducer('count_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['count' + label] = 'count_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'count_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitTimedCount(self, ctx:StreamParser.TimedCountContext):
        ...
        if 'tcount' + interval + label in self.already_seen:
            ...
            self.createAggregationtransducer('tcount_transducer_' + str(self.ntransducers), aggrFunc, self.type, label)
            self.already_seen['tcount' + interval + label] = 'tcount_transducer_' + str(self.ntransducers)
            self.ntransducers += 1
            return 'kwargs[\'tcount_transducer_' + str(self.ntransducers - 1) + '\']'
    ...
    def visitProp(self, ctx:StreamParser.PropContext):
        ...
        if name in self.types:
            ...
            transducerName = 'transducer_' + name

...
def callbackmtl_monitor_2(data):
	ws_lock.acquire()
	...
	conditional_publish()
	ws_lock.release()
def callbackmtl_monitor_5(data):
	ws_lock.acquire()
	if data.time not in dict_msgs:
		dict_msgs[data.time] = set()
...
def main(argv):
	global pub, monitor
	rospy.init_node('and_monitor_6', anonymous=True)
	pub = rospy.Publisher(name = 'and_monitor_6', data_class = TimedBool, latch = True, queue_size = 1000)
	rospy.Subscriber('mtl_monitor_2', TimedBool, callbackmtl_monitor_2)
	rospy.Subscriber('mtl_monitor_5', TimedBool, callbackmtl_monitor_5)
	rospy.spin()

...
class mtl_monitor_14(BaseMonitor):
	...
	def update(self, **kwargs):
		...
		self.states[0] = self.states[0] and kwargs['greater_monitor_13'];
...
def callbackgreater_monitor_13(data):
	ws_lock.acquire()
	if data.time not in dict_msgs:
		dict_msgs[data.time] = set()
	dict_msgs[data.time].add(('greater_monitor_13', data.value))
	conditional_publish()
	ws_lock.release()
...
def main(argv):
	global pub, monitor
	rospy.init_node('mtl_monitor_14', anonymous=True)
	monitor = mtl_monitor_14()
	rospy.Subscriber('greater_monitor_13', TimedBool, callbackgreater_monitor_13)
	pub = rospy.Publisher(name = 'mtl_monitor_14', data_class = TimedBool, latch = True, queue_size = 1000)
	rospy.spin()

...
class mtl_monitor_28(BaseMonitor):
	...
	def update(self, **kwargs):
		...
		return kwargs['velocity_turning_ok']
...
def callbackvelocity_turning_ok(data):
	ws_lock.acquire()
	if data.time not in dict_msgs:
		dict_msgs[data.time] = set()
	dict_msgs[data.time].add(('velocity_turning_ok', data.value))
	conditional_publish()
	ws_lock.release()
...
def main(argv):
	global pub, monitor
	rospy.init_node('mtl_monitor_28', anonymous=True)
	monitor = mtl_monitor_28()
	rospy.Subscriber('velocity_turning_ok', TimedBool, callbackvelocity_turning_ok)
	pub = rospy.Publisher(name = 'mtl_monitor_28', data_class = TimedBool, latch = True, queue_size = 1000)
	rospy.spin()

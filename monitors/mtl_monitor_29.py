...
class mtl_monitor_29(BaseMonitor):
	...
	def update(self, **kwargs):
		...
		return kwargs['velocity_forward_ok']
...
def callbackvelocity_forward_ok(data):
	ws_lock.acquire()
	if data.time not in dict_msgs:
		dict_msgs[data.time] = set()
	dict_msgs[data.time].add(('velocity_forward_ok', data.value))
	conditional_publish()
	ws_lock.release()
...
def main(argv):
	global pub, monitor
	rospy.init_node('mtl_monitor_29', anonymous=True)
	monitor = mtl_monitor_29()
	rospy.Subscriber('velocity_forward_ok', TimedBool, callbackvelocity_forward_ok)
	pub = rospy.Publisher(name = 'mtl_monitor_29', data_class = TimedBool, latch = True, queue_size = 1000)
	rospy.spin()

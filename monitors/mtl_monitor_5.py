...
class mtl_monitor_5(BaseMonitor):
...
def callbackmtl_monitor_4(data):
	ws_lock.acquire()
	if data.time not in dict_msgs:
		dict_msgs[data.time] = set()
	dict_msgs[data.time].add(('mtl_monitor_4', data.value))
	conditional_publish()
	ws_lock.release()
...
def main(argv):
	global pub, monitor
	rospy.init_node('mtl_monitor_5', anonymous=True)
	monitor = mtl_monitor_5()
	rospy.Subscriber('mtl_monitor_4', TimedBool, callbackmtl_monitor_4)
	rospy.Subscriber('greater_monitor_3', TimedBool, callbackgreater_monitor_3)
	pub = rospy.Publisher(name = 'mtl_monitor_5', data_class = TimedBool, latch = True, queue_size = 1000)
	rospy.spin()

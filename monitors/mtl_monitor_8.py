...
class mtl_monitor_8(BaseMonitor):
	...
	def update(self, **kwargs):
		...
		self.states[0] = self.states[0] and kwargs['greater_monitor_7'];
...
def callbackgreater_monitor_7(data):
	ws_lock.acquire()
	if data.time not in dict_msgs:
		dict_msgs[data.time] = set()
	dict_msgs[data.time].add(('greater_monitor_7', data.value))
	conditional_publish()
	ws_lock.release()
...
def main(argv):
	...
	rospy.Subscriber('greater_monitor_7', TimedBool, callbackgreater_monitor_7)

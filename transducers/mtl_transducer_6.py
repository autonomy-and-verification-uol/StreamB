...
class mtl_transducer_6(Basetransducer):
	...
	def update(self, **kwargs):
		...
		self.states[0] = self.update_timed_since(self.time, self.states[0], True, not kwargs['greater_eq_transducer_5'], 0, 5)
...
def callbackgreater_eq_transducer_5(data):
	ws_lock.acquire()
	if data.time not in dict_msgs:
		dict_msgs[data.time] = set()
	dict_msgs[data.time].add(('greater_eq_transducer_5', data.value))
	conditional_publish()
	ws_lock.release()
...
def main(argv):
	...
	rospy.init_node('mtl_transducer_6', anonymous=True)
	transducer = mtl_transducer_6()
	rospy.Subscriber('greater_eq_transducer_5', TimedBool, callbackgreater_eq_transducer_5)
	pub = rospy.Publisher(name = 'mtl_transducer_6', data_class = TimedBool, latch = True, queue_size = 1000)
	rospy.spin()
